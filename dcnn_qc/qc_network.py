...
class SlowdownCOVID19QCEngine():
    ...
    def conv_block(self, input_layer, n_filters, length=2, pool=True, stride=1):
        ...
        parallel = Conv2D(n_filters, (1, 1), strides=stride **
                          length, padding='same')(input_layer)
    ...
    def GenerateData(self, input_dir, output_h5, num_training=6000, num_validation=1200):
        ...
        X_train = ff.create_dataset(
            "X_train", (num_training,) + input_shape, dtype='float32')
        y_train = ff.create_dataset(
            "y_train", (num_training,), dtype='float32')

        X_val = ff.create_dataset(
            "X_val", (num_validation,) + input_shape, dtype='float32')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='QC X-ray images for COVID project')
    parser.add_argument('--operation', help="TRAIN / TEST, etc.", type=str, required=True,
                        choices=['TRAIN', 'TEST', 'DATAGENERATOR'])
    parser.add_argument('--output_folder', type=str,
                        help="Program output logging folder (additional)")
    parser.add_argument('--params_file', type=str,
                        help="Parameters file. Required for both TRAIN/TEST operations.")
    parser.add_argument('--input_dir', type=str,
                        help="Folder to use to create training/validation dataset")
    parser.add_argument('--training_h5', type=str,
                        help="H5 for training. This file is produce by the DATAGENERATOR")
    ...
    parser.add_argument('--model_w', type=str,
                        help="Model weights file for deployment")
    parser.add_argument('--out_csv', type=str,
                        help="Testing results: filename, class")

    args = parser.parse_args()

    current_folder = os.path.dirname(os.path.realpath(__file__))
    default_output_folder = os.path.realpath(
        os.path.join(current_folder, "..", "output"))
